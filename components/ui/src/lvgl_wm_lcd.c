...
/*
 * lvgl_wm_lcd.c — LCD LVGL window manager for ThistleOS
 *
 * Initializes the UI manager with the LCD flush callback and
 * deferred refresh disabled. Shows a splash screen on init.
 */
...
static const char *TAG = "lvgl_wm_lcd";
...
static esp_err_t lvgl_lcd_wm_init(void)
{
    ESP_LOGI(TAG, "LVGL LCD window manager initializing");
    esp_err_t ret = ui_manager_init(ui_flush_cb_lcd, false);
    if (ret != ESP_OK) {
    ...
    ESP_LOGI(TAG, "LVGL LCD window manager ready");
    return ESP_OK;
}
...
static bool lvgl_lcd_wm_on_input(const hal_input_event_t *e) { (void)e; return false; }

/* ── Public WM vtable ───────────────────────────────────────────────── */

static const display_server_wm_t s_lvgl_lcd_wm = {
    /* Lifecycle */
    .init              = lvgl_lcd_wm_init,
    .deinit            = lvgl_lcd_wm_deinit,
    .render            = lvgl_lcd_wm_render,
    .on_theme_changed  = lvgl_lcd_wm_on_theme_changed,
    .on_app_launched   = lvgl_lcd_wm_on_app_launched,
    .on_app_stopped    = lvgl_lcd_wm_on_app_stopped,
    .on_app_switched   = lvgl_lcd_wm_on_app_switched,
    .on_input          = lvgl_lcd_wm_on_input,

...
mod tests {
    ...
    fn test_flush_uses_padded_rows_for_three_by_two_rectangle() {
        ...
        // Each 3-pixel row occupies one byte; the low five bits are padding.
        ...
        let fb = unsafe { std::slice::from_raw_parts(fb_ptr(), EPD_FB_BYTES) };
        assert_eq!(fb[2 * (EPD_WIDTH / 8)], 0b0101_0000);
        assert_eq!(fb[3 * (EPD_WIDTH / 8)], 0b0010_0000);
    }
    ...
    fn test_flush_uses_padded_rows_for_nine_by_two_rectangle() {
        ...
        // Each 9-pixel row occupies two bytes; the second byte has seven pad bits.
